...
(function () {
  ...
  const POS_KEY = NS + '.position.' + slug;
  const READ_KEY = NS + '.read'; // JSON: { slug: pct, ... }
  ...
  if (tutorBtn && tutorEl) {
    ...
    const SYSTEM_PROMPT = "You are the in-book tutor for 'Building an Agentic Enterprise' " +
      "— a practitioner's report on deploying agentic AI inside real organizations. The book covers " +
      "process mapping from workflow to agent, the NIST AI Risk Management Framework " +
      "(Govern, Map, Measure, Manage) including the Generative AI Profile (NIST AI 600-1), enterprise " +
      "orchestration components (agent registry, memory management, tool catalogs, MCP, A2A, evals), " +
      "real case studies (Klarna, Moody's, JPMorgan, ServiceNow, Salesforce Agentforce, Air Canada, " +
      "Walmart), risk mitigation (OWASP LLM Top 10, MITRE ATLAS, prompt injection, autonomy creep, " +
      "agent IAM), ROI calculation, and no-code/low-code deployment (Copilot Studio, Agentforce, " +
      "ServiceNow AI Agents, n8n, Vertex Agent Builder, Bedrock Agents). Answer clearly and " +
      "concisely (3-6 sentences unless the user asks for depth). Be honest about hype vs reality " +
      "— if a claim is shaky, say so. Prefer concrete examples and real-world tradeoffs over " +
      "vendor narratives. If a question is outside the book's scope, still answer briefly but " +
      "note the connection back to the book. Never refuse a reasonable technical question. " +
    ...
    if (!log.children.length) {
      append('assistant', 'Hi — I\'m your tutor for this report. Ask me anything: how to run a NIST RMF Map workshop, what to put in an agent registry, why workflows aren\'t agents, or how to size HITL honestly. I\'ll keep it short and clear.');
    }
    ...
    async function askTutor(question, placeholder, attemptIdx) {
      ...
      try {
        ...
        const res = await fetch(endpoint.url, {
          ...
          body: JSON.stringify({ model: endpoint.model, messages, referrer: 'agentic-enterprise' }),
